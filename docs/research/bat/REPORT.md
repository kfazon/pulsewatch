# BAT Prodajni centar Čakovec — početni javni baseline

| Polje | Vrijednost |
|---|---|
| Točan scope | BAT Prodajni centar Čakovec, Rudolfa Steinera 2, HR-40000 Čakovec |
| Datum provjere | 17.08.2026. |
| Status | Demonstracijski početni baseline; nije povijesni before/after monitoring |
| Dokazi | Službene BAT stranice te službene stranice lokalnih konkurenata |
| Ograničenje | Online kategorija ili proizvod ne dokazuje fizičku zalihu, cijenu ni dostupnost u poslovnici |

## Izvršni sažetak

**Potvrđeno:** BAT javno navodi poslovnicu na adresi Rudolfa Steinera 2, s centralnim telefonom i odvojenim kontaktima za više prodajnih odjela.[2] Ljetno radno vrijeme poslovnice je ponedjeljak–petak 7:00–20:30 i subota 7:00–19:00.[5]

**Potvrđeno:** Javna ponuda kombinira širok asortiman, preko 80.000 artikala kao marketinški navod, specijalističke kontakte i dodatne usluge.[1][2][3]

**Potvrđeno:** Besplatna dostava iznad 1.000 € na području Međimurske županije vrijedi za robu osim građevinskog materijala; za građevinski materijal cijena dostave daje se na upit.[14]

**Potvrđeno:** Za 22.08.2026. objavljena je jednodnevna akcija od 15% na redovne cijene laminata, kosilica, boja i lakova te ručnih i električnih alata.[6] YATO aku i električni alat istodobno je na 20% rasprodaji do rasprodaje zaliha.[7] Gorenje promocija za odabrane perilice posuđa traje do 20.09.2026. ili do isteka zaliha.[15]

**Opažanje:** Stranica letaka navodi više tematskih kataloga, ali u dohvaćenom javnom tekstu uz nazive nisu bili vidljivi datumi valjanosti.[4] To je signal za provjeru vidljivosti i svježine, a ne dokaz da fizička poslovnica nema važeće ponude.[4]

**Hipoteza:** Najbrži PulseWatch pilot nije opći “monitoring svega”, nego tri zaključana toka: konkurentske akcije po kategoriji, status kataloga/letaka i do 20 usporedivih artikala ili projektnih košarica.[unverified]

## 1. Činjenice o poslovnici

| Oznaka | Područje | Nalaz | Dokaz |
|---|---|---|---|
| Potvrđeno | Lokacija | Rudolfa Steinera 2, HR-40000 Čakovec | [2] |
| Potvrđeno | Kontakt | +385 (0)40 389 900; bat@batprodajnicentar.hr | [2] |
| Potvrđeno | Radno vrijeme | Pon–pet 7:00–20:30; subota 7:00–19:00 | [5] |
| Potvrđeno | Odjelni kontakti | Info pult, građevina, elektro/rasvjeta, alati, bijela tehnika, boje, drvo, keramika/sanitarije, ljubimci, lov/ribolov, vrt | [2] |
| Potvrđeno | Plaćanje | Gotovina, avans po ponudi, virman, kartice i 2–12 rata | [13] |
| Potvrđeno | Dostava | Besplatna iznad 1.000 € na području Međimurske županije za robu osim građevinskog materijala; građevinski materijal na upit | [14] |
| Ograničenje | Fizička zaliha | Nije potvrđena iz javnog web prikaza | — |

## 2. Asortiman i usluge

BAT javno navodi dostavu na kućni prag, 3D dizajn kupaonice, miješanje/nijansiranje boja, rezanje i kantiranje drva, preuzimanje otpadne ambalaže, cvjetnu/poklon galeriju, caffe bar i kiosk brze hrane.[3]

| Kupovni zadatak | Javno vidljiva sposobnost | Poslovna važnost | Status |
|---|---|---|---|
| Gradnja i renovacija | Građevinski odjel, drvo, keramika, sanitarije, boje i alati | Visoka vrijednost košarice i potreba za savjetom | Potvrđeno |
| Vrt i okućnica | Vrtni centar, kosilice, vrtni katalozi | Sezonalnost i lokalna konkurencija | Potvrđeno |
| Dom i tehnika | Bijela/audio-video tehnika i široka potrošnja | Akcije, pokloni i cjenovna usporedba | Potvrđeno |
| Projektna usluga | 3D kupaonica, miješanje boja, rezanje/kantiranje | Diferencijacija od čistog webshopa | Potvrđeno |
| Online dostupnost | Web kategorije i košarica | Indikator digitalnog asortimana | Ne dokazuje zalihu poslovnice |

## 3. Aktualni javni signali

| Prioritet | Signal | Poslovno značenje | Preporučena akcija |
|---|---|---|---|
| VISOKO | Narančasta subota 22.8.: 15% na pet skupina proizvoda | Kratak prozor traži sinkronizaciju weba, newslettera, društvenih kanala i prodajnog mjesta | Provjeriti objavu, uvjete i uklanjanje nakon isteka; izmjeriti promet i bruto dobit po kategoriji |
| VISOKO | YATO alat -20% do rasprodaje zaliha; artikli izlaze iz asortimana | Signal čišćenja asortimana i mogućeg prelaska kupaca na zamjenski brend | Pratiti dostupnost, uklanjanje akcije i definirati zamjenski asortiman |
| SREDNJE | Gorenje perilica + ledomat na poklon do 20.9. ili isteka zaliha | Proizvođačka promocija veže kupnju, račun i naknadnu prijavu | Pratiti rok, uvjete, uklanjanje objave i jasnoću kupčeva slijeda do poklona |
| SREDNJE | Tematski katalozi bez vidljivih datuma u izvučenom tekstu | Kupac teže procjenjuje je li katalog aktualan | Uz svaki katalog prikazati datum valjanosti i status “aktualno/arhiva” |
| SREDNJE | Nije javno potvrđen jedinstveni rok provjere zalihe i rezervacije | Kupac prije dolaska možda mora zvati pojedini odjel | Pilotirati “provjeri i rezerviraj” za jednu projektnu kategoriju |

## 4. Lokalni konkurentski okvir

Grama u Čakovcu javno kombinira fizički centar, odjelne kontakte i webshop za dom, poljoprivredu, vrt i okoliš.[8][9]

Petrokov ima lokalnu poslovnicu u Josipa Kozarca 21 i specijalistički okvir grijanja, hlađenja, instalacija i sanitarija.[10]

Vrtni centar Iva u Štefancu specijaliziran je za bilje, sjeme, tegle, gnojiva, supstrate i vrtni alat te javno prikazuje webshop.[11][12]

| Konkurent | Dokazivo preklapanje | Što pratiti | Ograničenje |
|---|---|---|---|
| Grama Čakovec | Gradnja, vrt, poljoprivreda, okoliš, webshop | Katalozi, sezonske akcije, lokalna usluga/preuzimanje | Nije napravljena EAN-identična cjenovna košarica |
| Petrokov Čakovec | Grijanje, hlađenje, instalacije, sanitarije | Stručne ponude, projektna prodaja, akcije po brandu | Uži specijalistički format, nije usporediv s cijelim BAT-om |
| Vrtni centar Iva | Bilje, vrt, gnojiva, supstrati, alati, webshop | Sezonska dostupnost, click/preuzimanje, sadržaj i savjeti | Uži vrtni specijalist |
| BAUHAUS Varaždin | Regionalni veliki DIY format | Usluge, rezervacija/preuzimanje, akcije i projektne kategorije | Službena lokacijska stranica bila je blokirana ovom dohvatom; detalji nisu korišteni kao dokaz |

## 5. Operativne hipoteze za test

| Opažljivi signal | Hipoteza | Pitanje za validaciju | Pilot i metrika |
|---|---|---|---|
| Brojni odjelni kontakti | Stručni odgovor je BAT-ova prednost, ali kupovni put ovisi o pozivu | Koliki je odgovor i konverzija upita po odjelu? | Jedinstvena evidencija upita; vrijeme odgovora i prodajna konverzija |
| Kratke višekategorijske akcije | Dio učinka ovisi o pravodobnoj distribuciji na svim kanalima | Jesu li svi kanali aktivni prije početka i uklonjeni nakon završetka? | Before/after capture; pokrivenost kanala i promet po kategoriji |
| Projektne usluge | 3D, boje i obrada drva mogu braniti maržu bolje od općeg popusta | Koliki udio upita prelazi u ponudu i kupnju? | 30 dana; broj upita, ponuda, konverzija i prosječna košarica |
| Katalozi bez jasne valjanosti u izvučenom tekstu | Jasna svježina može smanjiti zabunu kupca | Koliko poziva ili odustajanja nastaje zbog nejasne aktualnosti? | Status i datum na svim katalozima; klikovi i upiti |
| Nema javno potvrđenog branch-stock SLA-a | Brza potvrda dostupnosti može smanjiti odlazak konkurentu | Koje kategorije mogu pouzdano potvrditi zalihu bez ERP integracije? | Jedna kategorija; odgovor u dogovorenom roku i stopa preuzimanja |

## 6. Realno u dosegu direktora poslovnice

**Vjerojatno lokalno ili brzo odobrenje:** raspored odgovornosti za upite, standard odgovora odjela, lokalna koordinacija kampanje, provjera oznaka i letaka, pilot evidencije upita te tjedni pregled signala.[unverified]

**Vjerojatno traži upravu ili centralno odobrenje:** politika cijena i nabave, ERP/POS integracija, osobni podaci, promjena uvjeta dostave/povrata, veći marketinški budžet, program vjernosti i ugovori s dobavljačima.[unverified] Ovo je radna hipoteza dok se ne potvrde interne ovlasti.[unverified]

## 7. Prijedlog 30-dnevnog PulseWatch pilota

| Element | Scope |
|---|---|
| BAT izvori | Naslovnica, akcije, letci, odabrane kategorije, usluge, plaćanje, dostava i radno vrijeme |
| Konkurenti | Grama, Petrokov, Vrtni centar Iva i BAUHAUS samo gdje je službeni dokaz dostupan |
| Cijene | Do 20 unaprijed zaključanih identičnih artikala/pakiranja ili projektnih košarica |
| Alert | Samo materijalna promjena uz timestamp, before/after dokaz, važnost i preporučenu akciju |
| Izvještaj | Tjedni kratki signal + završni executive PDF |
| Kriterij uspjeha | Najmanje jedan pravodoban signal dovodi do dokumentirane komercijalne ili zaštitne akcije |

## 8. Pitanja za sastanak

1. Koje tri kategorije nose najviše prihoda ili marže, a koje najviše prometa?
2. Kako se danas potvrđuje zaliha i rezervira roba prije dolaska kupca?
3. Tko objavljuje akcije i kako se provjerava da su web, newsletter i prodajno mjesto usklađeni?
4. Koliko upita dobivaju 3D kupaonica, miješanje boja i rezanje/kantiranje te koliko ih prelazi u kupnju?
5. Koji konkurentski potez bi zahtijevao odgovor unutar jednog radnog dana?
6. Koji podaci mogu biti dostupni za mjerenje pilota bez pristupa osobnim podacima?

## Metodologija i ograničenja

Korišteni su samo javni izvori provjereni 17.08.2026.[unverified] Nisu korišteni interni promet, marže, zalihe, CRM, ugovori ni prodajni rezultati.[unverified] Nije provedena fizička posjeta ni tajna kupnja.[unverified] Online asortiman nije tretiran kao dokaz fizičke zalihe.[unverified] “Nije pronađeno” znači samo da tvrdnja nije potvrđena na pregledanim stranicama.[unverified] Početni baseline nije dokaz automatski detektirane povijesne promjene.[unverified]

## Sources

[1] https://batprodajnicentar.hr — BAT Prodajni centar — naslovnica
    > "Besplatna dostava Za sve kupnje iznad 1000€ na području Međimurja"
    > "Veliki izbor proizvoda Preko 80.000 artikala u ponudi"
[2] https://batprodajnicentar.hr/kontakt — BAT kontakt i odjeli
    > "Poslovnica Čakovec Adresa Rudolfa Steinera 2, HR-40000 Čakovec Telefon +385 (0)40 389 900 E-mail bat@batprodajnicentar.hr"
    > "Maloprodaja Info pult 040 389 916 info@batprodajnicentar.hr Građevinski materijal 040 389 921 gradjevina@batprodajnicentar.hr"
[3] https://batprodajnicentar.hr/dodatne-usluge — BAT dodatne usluge
    > "Dostava na kućni prag Opširnije 3D dizajn i vizualizacija Vaše preuređene kupaonice Opširnije Miješanje / nijansiranje boja Opširnije Rezanje drva i kantiranje"
[4] https://batprodajnicentar.hr/aktualni-letci — BAT letci
    > "Pregledajte naše aktualne letke i tematske kataloge i ne propustite najnoviju ponudu i akcije!"
[5] https://batprodajnicentar.hr/2026/03/26/ljetno-radno-vrijeme-u-bat-u — BAT ljetno radno vrijeme
    > "Prodajni centar Čakovec • Ponedjeljak – petak: 7:00 – 20:30 • Subota: 7:00 – 19:00"
[6] https://batprodajnicentar.hr/2026/08/17/narancasta-subota-22-8-laminati-kosilice-boje-lakovi-alat — BAT Narančasta subota 22.8.
    > "U subotu, 22. kolovoza , stiže nova Narančasta subota! Iskoristite 15% popusta na redovne cijene laminata, kosilica, boja i lakova te ručnih i električnih alata ."
[7] https://batprodajnicentar.hr/2026/08/06/yato-alat-20-posto-popusta-bat — BAT YATO rasprodaja
    > "Akcija traje: od 4. 8. 2026. do rasprodaje zaliha."
[8] https://grama.com.hr — Grama naslovnica
    > "Preko našeg web shopa možete kupovati različite proizvode za uređenje doma, preparate i gnojiva za poljoprivredu i vrt te različite proizvode za uređenje okoliša vašeg doma."
[9] https://grama.com.hr/kontakt — Grama kontakt
    > "Grama Ul. Zrinsko Frankopanska 15 40000, Čakovec Tel: 040 328 720 E-mail: info@grama-trgovina.com"
[10] https://petrokov.hr/poslovnica-cakovec — Petrokov poslovnica Čakovec
    > "Poslovnica Čakovec Josipa Kozarca 21, 40000 Čakovec, HR +385 (0)40 296 014 poslovnica.cakovec@petrokov.hr Radno vrijeme: pon-pet 07-16h, sub NE RADI"
[11] https://vrtnicentariva.hr — Vrtni centar Iva
    > "Osim šume biljaka imamo i sve što pomaže u njihovom uzgoju. Biljke i biljčice, sjeme i sjemenčice, tegle, teglice, gnojiva, supstrati i svi mogući alati."
[12] https://vrtnicentariva.hr/kontakt — Vrtni centar Iva kontakt
    > "Poslovnica Štefanec Braće Radića 54, Štefanec 40000 Čakovec"
[13] https://batprodajnicentar.hr/nacini-i-pogodnosti-placanja — BAT načini i pogodnosti plaćanja
    > "Načini plaćanja: gotovinom prilikom kupnje na blagajni prodajnog centra ili poslovnice gotovinom ili avansno na temelju ponude virmanom kartičnim plaćanjem Pogodnosti plaćanja: na rate, od 2 do 12 rata"
[14] https://batprodajnicentar.hr/opci-uvjeti-poslovanja — BAT opći uvjeti poslovanja
    > "Dostava sve robe, osim građevinskog materijala, na kućni prag - u iznosu iznad 1.000 € – je na području Međimurske županije - besplatna. Cijenu dostave građevinskog materijala, ovisno o količini, vrijednosti i načinu isporuke, dajemo na upit."
[15] https://batprodajnicentar.hr/2026/08/11/gorenje-perilica-posuda-ledomat — BAT Gorenje perilica + ledomat
    > "Promocija traje od 20. srpnja do 20. rujna 2026. ili do isteka zaliha"
