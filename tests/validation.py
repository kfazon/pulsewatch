...
def test_diff_consistency(sample_diff):
    ...
    diff_generator = unified_diff(
        capture_old.splitlines(),
        capture_new.splitlines()
    )
    calculated_diff = '\n'.join(list(diff_generator))
    assert calculated_diff.strip() == diff_text.strip(), "Calculated diff does not match provided diff text"
...
def test_summary_json_valid(sample_capture_dir):
    ...
    for field in required_fields:
        assert field in summary_data, f"Missing required field '{field}' in summary JSON"
    
    assert isinstance(summary_data["changed_text"], str), "changed_text must be a string"
    assert isinstance(summary_data["importance_score"], (int, float)), "importance_score must be a number"
